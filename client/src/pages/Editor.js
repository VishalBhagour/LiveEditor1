// pages/Editor.js — Real-time collaborative code editor
...
// ── Code runner — browser for JS/HTML, YOUR backend for everything else ────────
const executeCode = async (code, language) => {
  ...
  // ── Python / Java / C++ / TypeScript: YOUR backend ──────────
  try {
    const { data } = await API.post('/execute', { code, language });

    // If HTML returned (shouldn't happen for these langs, but just in case)
    if (data.isHtml) {
    ...
    // Split output by lines and detect errors
    output.split('\n').forEach((line) => {
      if (!line && logs.length > 0) return; // skip trailing empty lines
      const isError =
